...
function classify(m,s={}){const c=num(s.currentCpc||m.currentCpc),make=(code,label,stepPct,reason,evidence='Низкая')=>({code,label,stepPct,reason,evidence,targetCpc:c>0?c*(1+stepPct):null}),conf=num(m.confidence),pairs=num(m.informativePairs),coverage=num(m.orderCoverage),eo=Number(m.eOrder),adj=Number(m.adjustedEOrder),ec=Number(m.eCart),ac=Number(m.adjustedECart),po=finite(m.orderPValue)?Number(m.orderPValue):1,pc=finite(m.cartPValue)?Number(m.cartPValue):1,lag=Number(m.lagAdjustedEOrder),lp=finite(m.lagOrderPValue)?Number(m.lagOrderPValue):1,max=finite(s.maxIncrementalCpo)&&Number(s.maxIncrementalCpo)>0?Number(s.maxIncrementalCpo):null,mar=Number(m.marginalCpo);
